...
_(verbatim — by state, since this component is essentially a stateful drop target)_
...
**Anatomy** _(derived from the state list + O7 Step 1 spec: "Drop zone for XML file or QR image" + "OR paste Alight Motion link"):_
...
### Props — _[GAP FILL — not specified in source, derived from O7 UploadFormData + Product Spec §4 `presets.file_type`]_
...
_(verbatim list, expanded into full visual treatment)_
...
_(verbatim — by behavior, since this component has no visual variants, only a single canonical pattern)_
...
**Anatomy** _(derived from the behavior list + A6 Tag's existing chip treatment, referenced by O7 Step 2: "Tags (A6 Tag via CC2 Tag Input, max 10)"):_
...
### Props — _[GAP FILL — not specified in source, derived from anatomy + Product Spec §4 `presets.tags`]_
...
_(verbatim — appearance, single pattern)_

**Appearance** _(verbatim)_:
...
### Props — _[GAP FILL — not specified in source, derived from anatomy + Product Spec §4 `presets.category` and §9 Category Colors]_
...
**Toolbar** _(verbatim):_ Bold · Italic · Link · Unordered list · Ordered list

**Tabs** _(verbatim):_ Write | Preview (live rendered markdown)

**Output** _(verbatim):_ Stored as markdown, rendered via `react-markdown` + `remark-gfm`.

**Security** _(verbatim):_ Rendered markdown sanitized via DOMPurify.

**Anatomy** _(derived from the toolbar + tabs spec, cross-referenced with A3 Textarea's existing markdown note: "`description?: string // Markdown, rendered via react-markdown + DOMPurify`" in `02-molecules.md` M5 NotificationItem props)_:
...
### Props — _[GAP FILL — not specified in source, derived from anatomy + Product Spec §4 `presets.description` (markdown, max 2000 chars)]_
...
_(verbatim — features list, single pattern)_
...
**Anatomy** _(derived from the features list + F2 Modal's `fullscreen` size, which Design System §17 F2 explicitly names this component as an example of: "Immersive content — image cropper (CC5)"):_
...
### Props — _[GAP FILL — not specified in source, derived from anatomy + Product Spec §12 Upload Architecture / Cloudinary Transformations]_
...
_(verbatim — layout, single pattern, by mode)_

**Layout** _(verbatim):_ Stack of `mini` CreatorCards, load more at bottom.

**Modes** _(derived from the Purpose statement "followers or following" + Product Spec §4 `follows` table, which is symmetric — the same list UI serves both directions):_

- `followers` — users who follow the profile being viewed
- `following` — users the profile being viewed follows

### Props — _[GAP FILL — not specified in source, derived from anatomy + Product Spec §4 `follows` table]_
...
_(verbatim — structure, single pattern)_

**Structure** _(verbatim):_
...
**Anatomy** _(derived from the structure list + Product Spec §6.7 Explore Page, which shares most of the same elements):_
...
### Props — _[GAP FILL — not specified in source, derived from anatomy + Product Spec §16 Search endpoint]_
...
_End of Compound Components — this is the final file in the PresetHub UI Component Library set (`00`–`06`)._

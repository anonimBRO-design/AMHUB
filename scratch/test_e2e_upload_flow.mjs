...
async function testE2E() {
	...
	const resSlug = await fetch(
		`${url}/rest/v1/presets?select=${encodeURIComponent(PRESET_SELECT_WITH_CREATOR)}&slug=eq.malam-pagi-1599&status=eq.published`,
		{
			headers: {
				apikey: anonKey,
	...
	const resCatalog = await fetch(
		`${url}/rest/v1/presets?select=${encodeURIComponent(PRESET_SELECT_WITH_CREATOR)}&status=eq.published&order=created_at.desc&limit=24`,
		{
			headers: {
				apikey: anonKey,
				Authorization: `Bearer ${anonKey}`,
	...
	const resCreator = await fetch(
		`${url}/rest/v1/presets?select=${encodeURIComponent(PRESET_SELECT_WITH_CREATOR)}&creator_id=eq.${creatorId}&order=created_at.desc`,
		{
			headers: {
				apikey: anonKey,
				Authorization: `Bearer ${anonKey}`,

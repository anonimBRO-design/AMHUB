...
async function checkPresets() {
  ...
  const resSelect = await fetch(`${url}/rest/v1/presets?select=${encodeURIComponent(selectQuery)}&limit=5`, {
    headers: {
      'apikey': serviceKey,
      'Authorization': `Bearer ${serviceKey}`,
    }
  });
  console.log("PRESET_SELECT_WITH_CREATOR status:", resSelect.status, resSelect.statusText);
  ...
  const resAnon = await fetch(`${url}/rest/v1/presets?select=${encodeURIComponent(selectQuery)}&status=eq.published&limit=5`, {
    headers: {
      'apikey': anonKey,

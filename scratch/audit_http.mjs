...
async function testFetch() {
  ...
  console.log("Service Content-Range:", resService.headers.get('content-range'));
  ...
  console.log("Service Select Star Status:", resServiceStar.status, resServiceStar.statusText);
  ...
  const resAnon = await fetch(`${url}/rest/v1/users?select=id,username,display_name,avatar_url,is_verified`, {
    headers: {
      'apikey': anonKey,
  ...
  console.log("Anon Select Star Status:", resAnonStar.status, resAnonStar.statusText);

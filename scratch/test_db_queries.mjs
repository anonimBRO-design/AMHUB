...
async function testAll() {
	...
	const resSlug = await fetch(
		`${url}/rest/v1/presets?select=${encodeURIComponent(SAFE_SELECT)}&slug=eq.malam-pagi-1599&status=eq.published`,
		{
			headers: {
				apikey: anonKey,
	...
	const resList = await fetch(
		`${url}/rest/v1/presets?select=${encodeURIComponent(SAFE_SELECT)}&status=eq.published&order=created_at.desc&limit=10`,
		{
			headers: {
				apikey: anonKey,
				Authorization: `Bearer ${anonKey}`,

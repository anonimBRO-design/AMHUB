...
async function runDiagnostics() {
  ...
  const serviceRes = await serviceClient.from('users').select('id', { count: 'exact' });
  ...
  const serviceStarRes = await serviceClient.from('users').select('*', { count: 'exact' });
  ...
  const anonRes = await anonClient.from('users').select('id', { count: 'exact' });
  ...
  const anonHomeRes = await anonClient.from('users').select('id, username, display_name, avatar_url, is_verified');
  ...
  try {
    const { data: grants, error: grantsErr } = await serviceClient.rpc('exec_sql', {

...
export function Hero({ stats, featuredPreset }: HeroProps) {
	...
	return (
		<section className="relative overflow-hidden pt-8 pb-12 md:py-16 px-6 sm:px-10 rounded-3xl bg-[var(--color-bg-surface)] border border-[var(--color-border-subtle)] shadow-[0_8px_32px_rgba(0,0,0,0.3)] transition-all duration-300">
			{/* Subtle Background Elements */}
			<div className="pointer-events-none absolute -top-40 left-1/3 w-[500px] h-[500px] bg-[var(--color-bg-elevated)] rounded-full animate-ambient-float" />
			<div className="pointer-events-none absolute -bottom-40 right-10 w-[450px] h-[450px] bg-[var(--color-bg-elevated)] rounded-full animate-glow-pulse" />
			<div className="pointer-events-none absolute top-1/2 left-0 -translate-y-1/2 w-64 h-64 bg-[var(--color-bg-accent)] rounded-full" />
			...
			<div className="relative z-10 max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
				...
				<div className="lg:col-span-7 space-y-6 text-center md:text-left">
					...
					<div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center md:justify-start gap-4 pt-2">
						<Link
							href="/explore"
							className="inline-flex items-center justify-center gap-2.5 min-h-[52px] px-8 rounded-2xl bg-[var(--color-interactive-primary)] text-white font-extrabold text-sm shadow-[0_8px_24px_rgba(0,0,0,0.2)] hover:opacity-95 active:scale-[0.97] transition-all duration-300 hover:-translate-y-0.5"
						>
						...
						<Link
							href="/upload"
							className="inline-flex items-center justify-center gap-2.5 min-h-[52px] px-8 rounded-2xl bg-[var(--color-bg-elevated)] text-[var(--color-text-primary)] font-extrabold text-sm border border-[var(--color-border-subtle)] hover:bg-[var(--color-bg-elevated)]/80 active:scale-[0.97] transition-all duration-300 hover:-translate-y-0.5"
						>
					...
					<div className="grid grid-cols-3 gap-3 sm:gap-4 pt-6 border-t border-[var(--color-border-subtle)] max-w-lg mx-auto md:mx-0">
				...
				<div className="lg:col-span-5 relative flex items-center justify-center pt-4 lg:pt-0">
					<div className="relative w-full max-w-md aspect-[4/5] sm:aspect-square lg:aspect-[4/5] flex items-center justify-center">
						{/* Background Card */}
						<div className="absolute top-4 -right-2 w-72 h-96 rounded-3xl bg-[var(--color-bg-elevated)] border border-[var(--color-border-subtle)] shadow-xl rotate-6 transform transition-transform duration-500 hover:rotate-3 hidden sm:block pointer-events-none" />

						{/* Main Featured Showcase Glass Card */}
						<div className="relative z-20 w-full rounded-3xl bg-[var(--color-bg-surface)] border border-[var(--color-border-default)] p-5 shadow-[0_16px_48px_rgba(0,0,0,0.5)] transition-transform duration-500 hover:scale-[1.02] space-y-4">
							{featuredPreset ? (
								<Link
								...
								>
									...
									<div className="relative aspect-[16/10] w-full rounded-2xl overflow-hidden bg-[var(--color-bg-elevated)] border border-[var(--color-border-subtle)]">
										...
										<div className="absolute top-3 left-3 flex items-center gap-2">
											<span className="px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-black/80 text-white border border-white/10 shadow-md flex items-center gap-1">
												<Flame className="w-3 h-3 text-rose-400 fill-rose-400" />
												FEATURED
											</span>
											<span className="px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider bg-[var(--color-interactive-primary)] text-white border border-white/10 shadow-md uppercase">
										...
										<div className="absolute inset-0 flex items-center justify-center">
											<div className="w-12 h-12 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-white shadow-xl group-hover:scale-110 transition-transform duration-300">
												<Play className="w-5 h-5 fill-white translate-x-0.5" />
											</div>
										</div>

										<div className="absolute bottom-2.5 right-3 px-2.5 py-1 rounded-full bg-black/80 text-emerald-400 text-[11px] font-bold border border-emerald-500/20 flex items-center gap-1">
						...
						{/* Floating Decorative Badge 2 (Bottom Right) */}
						<div className="absolute -bottom-5 -right-4 z-30 px-4 py-2.5 rounded-2xl bg-[var(--color-bg-elevated)] border border-emerald-500/30 text-white shadow-xl flex items-center gap-2.5 animate-glow-pulse">

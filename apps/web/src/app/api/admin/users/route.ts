...
export async function DELETE(request: NextRequest) {
	try {
		...
		if (!deleteSuccess) {
			...
			const errObj = deleteErr as { message?: string; code?: string } | null;
			const rawMsg = errObj?.message || "";
			let userMsg = rawMsg || "Failed to delete user profile record";

			if (rawMsg.includes("schema cache") || errObj?.code === "PGRST202") {
				userMsg =
					"Database migration required: Function public.admin_delete_user is not installed in Supabase. Please run 20260815000000_admin_users_rpc_permissions.sql in Supabase SQL Editor.";
			} else if (

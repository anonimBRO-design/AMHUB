...
_(verbatim from Design System §17)_

Wraps all authenticated routes.

### Variants

_(no variants — single shell; the interior organisms O1/O2/O3 adapt by breakpoint)_
...
**Right Panel anatomy** _(from Product Spec §6.2 Home Feed, §9 Responsive Breakpoints):_
...
- **Skip link:** The first focusable element in the DOM, before any navigation chrome, is the skip link specified in Design System §13:
  ...
  <a
    href="#main-content"
    class="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[9999] focus:bg-accent-600 focus:text-white focus:px-4 focus:py-2 focus:rounded-md"
  >
...
### Responsive Behavior — _[GAP FILL, with reference to Design System §8 and §9]_
...
_(no variants — single shell, with contextual header behaviour)_
...
**PublicHeader anatomy** _(derived from Product Spec §6.1 Landing Page structure and §3 Navigation Structure):_
...
**Footer anatomy** _(derived from Product Spec §3 Site Map — no footer explicitly specified in source docs):_
...
| ---------- | --------------------------------------------------------------------- | ---------------------------------- | ------------------------------------------- |
| `xs`/`sm`  | Logo + **Hamburger button** + "Get Started" CTA _(ADR-025, RESOLVED)_ | Full width, `padding-x: --space-4` | Single column, stacked link groups          |
...
_(no variants — single split-panel pattern used for both login and register)_
...
**AuthPanel — left side** _(derived from Product Spec §3 auth routes: `/auth/login`, `/auth/register`, `/auth/callback`):_
...
**VisualPanel — right side** _(from Design System §17 T3 "Right: animated preset showcase"):_
...
_(derived from Product Spec §7 and §15)_
...
_(no variants defined in source — single pattern)_

**Structure** _(derived from Product Spec §3 Site Map admin routes and §15 Moderation Dashboard):_
...
**AdminSidebar anatomy** _(from Product Spec §3 Site Map admin routes and §15 Moderation System):_
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------- | --------------------- |
| `xs`/`sm`  | Full width  | **Desktop-only interstitial rendered** — AdminSidebar and main content replaced _(ADR-026, RESOLVED)_ | —                     |
...
_End of Templates — continued in `05-overlays-feedback.md`_

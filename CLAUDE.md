...
**Critical:** nearly every DAL function wraps its query in `try/catch` and, on any error _or empty result_, silently returns fabricated rows from `apps/web/src/data/mock-data.ts` (`MOCK_PRESETS`, `MOCK_CREATORS`, `MOCK_LIKES`, `MOCK_BOOKMARKS`, `MOCK_COMMENTS`, `MOCK_NOTIFICATIONS`). Examples: `getPresetBySlug` returns `MOCK_PRESETS[0]` for any unknown slug; `getUserByUsername` falls back to `MOCK_CREATORS[0]`. **If you see data that looks "too good" or fake counts (e.g. 48,500 followers), it's the mock fallback — the DB query failed.** When working on data code, treat the mock fallback as technical debt, not a feature. The mock dataset uses `Math.random()`/`Date.now()` at module load, so it also differs between server and client renders.

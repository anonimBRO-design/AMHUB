...
_(ADR-010, RESOLVED — formalizes the gradient previously only described in Product Spec §9; uses `--color-interactive-primary` and `--color-accent-400` as its endpoints.)_
...
- `md` (touch device, `@media (pointer: coarse)`): Icon-only sidebar collapsed; tap opens full-width (220px) sidebar as a drawer overlay (`slideInRight`, `--z-overlay: 300`). Backdrop tap collapses. Detection and conditional rendering handled in T1 AppLayout. _(ADR-018, RESOLVED)_
...
**Mobile search behavior (`xs`/`sm`):** _(ADR-019, RESOLVED)_
...
_PresetHub Design System — Motion Studio v1.0_  
_Do not alter tokens or component specs without design team review._  

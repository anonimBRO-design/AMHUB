...
_(verbatim — by breakpoint state)_
...
### Responsive Behavior — _[verbatim from Design System §17 O1 breakpoint table, expanded]_
...
_(no variants defined in source — single pattern with scroll behavior)_
...
| Search focused                | On `md+`: SearchBar receives focus ring inline. On `xs`/`sm`: `Search` icon button tap opens full-width overlay (`fadeIn` + `scaleIn`, `--z-overlay: 300`) containing M6 SearchBar; overlay close restores focus to the icon button. _(ADR-019, RESOLVED)_ |
...
| ---------- | --------------------------- | ---------------------------------------------------------------------------- | ------------------------- | ---------------------- |
| `xs`/`sm`  | Logo only (wordmark)        | `Search` icon button → opens full-width overlay on tap _(ADR-019, RESOLVED)_ | Bell icon only (no label) | Avatar only (no label) |
...
### Responsive Behavior — _[verbatim from Design System §8 Preset Grid table]_
...
### Motion Behavior — _[GAP FILL, mostly specified in Product Spec §11 and Design System §11]_
...
_(no variants — single layout pattern)_
...
**Page structure** _(from Product Spec §6.3):_
...
### Props — _[GAP FILL — not specified in source, derived from Product Spec §4 presets table and §6.3 page spec]_
...
| `lg+`      | 60/40 split (per spec)                                                                              | Metadata column `position: sticky; top: 64px` _(ADR-020, RESOLVED)_ |
...
_(no variants — single pattern; own profile vs. other profile differs by available actions, not layout)_

**Anatomy** _(verbatim):_
...
### Props — _[GAP FILL — not specified in source, derived from Product Spec §4 users table and §6.4 Profile Page spec]_
...
| ---------- | ------------------------------------------------------ | ------------------------------------------------------------------------- | ---------------- | ---------------------------- |
| `xs`/`sm`  | Full width, **3:1** aspect ratio _(ADR-021, RESOLVED)_ | `size="xl"` (64px) — reduced from 3xl to avoid dominating narrow viewport | Wrap to 2×2 grid | Stack vertically, full-width |
...
_(verbatim — by step)_

**Step indicator** _(verbatim):_

```
① Files  ──  ② Details  ──  ③ Preview
```

**Step 1 — Files** _(verbatim):_
...
### Responsive Behavior — _[GAP FILL, with reference to Design System §8 Upload Wizard grid spec]_
...
### Motion Behavior — _[GAP FILL, specified in Product Spec §11]_
...
_(no variants — single pattern)_
...
### Props — _[GAP FILL — not specified in source, derived from Product Spec §4 comments table and §13 API]_
...
_(no structural variants — tab-based sub-sections are routes, not component variants)_
...
**Header copy pattern** _(from Product Spec §6.6):_
...
### Props — _[GAP FILL — not specified in source, derived from Product Spec §6.6 and §13 Analytics API]_
...
### Responsive Behavior — _[GAP FILL, with reference to Design System §8 Dashboard Layout]_
...
- **Internal components:** M3 StatCard ×4 (stats row), Recharts `AreaChart` _(ADR-022, RESOLVED)_, M1 PresetCard `variant="compact"` (top preset mini-card), M5 NotificationItem-derived recent activity feed, A5 Badge `variant="status"` (preset status in table), F3 Dropdown (row actions), F6 Confirmation Dialog (delete confirm).
...
_(verbatim — by challenge status)_
...
### Props — _[GAP FILL — not specified in source, derived from Product Spec §4 challenges table and §6.8 Challenges Page]_
...
### Props — _[GAP FILL — not specified in source, derived from Product Spec §19 Leaderboard spec]_
...
### Motion Behavior — _[GAP FILL, partially specified in Product Spec §19 and M2 CreatorCard motion spec]_
...
_End of Organisms — continued in `04-templates.md`_
